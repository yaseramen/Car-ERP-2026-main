...
interface Props {
  supplierId: string;
  initialData: {
    supplier: { id: string; name: string; phone?: string | null; email?: string | null; address?: string | null; notes?: string | null };
    ...
    summary: { totalPurchases: number; totalPaid: number; totalBalance: number; invoiceCount: number; pendingCount: number };
  };
}

export function SupplierAccountContent({ supplierId, initialData }: Props) {
  const { supplier, invoices, summary } = initialData;

  return (
    <div className="space-y-6">
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-2">{supplier.name}</h1>
        <div className="text-sm text-gray-600 dark:text-gray-400 space-y-1">
          {supplier.phone && <p>📞 {supplier.phone}</p>}
          {supplier.email && <p>✉️ {supplier.email}</p>}
          {supplier.address && <p>📍 {supplier.address}</p>}
          {supplier.notes && <p className="mt-2">ملاحظات: {supplier.notes}</p>}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-100 dark:border-gray-700">
          ...
          <p className="text-xl font-bold text-gray-900 dark:text-gray-100">{summary.totalPurchases.toFixed(2)} ج.م</p>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-100 dark:border-gray-700">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
        <div className="p-4 border-b border-gray-100 dark:border-gray-700">
          <h2 className="font-bold text-gray-900 dark:text-gray-100">كشف فواتير الشراء</h2>
        </div>
